...
export default class Recipe extends Component{
  componentDidMount() {
    ...
    this.props.loadRecipeData(this.props.match.params.id)
  }
  ...
  render(props) {
    return(
      ...
      {this.props.recipe_data === '' ?
        ...
        this.props.recipe_data.map(item =>
          <div key={item._id} className="container max-width-90 min-width-90">
            ...
            <div className="pre-wrap margin-top-20 d-flex justify-content-center padding-10">
              {item.text}
            </div>
            <div onClick={this.props.toggleClassLastVersion}
              className="btn btn-secondary margin-20">
              Last version
            </div>
            <div className={this.props.show_last_version === true ? 'show h-100' : 'hide'}>
              ...
              <div>
                {item.lastVersions.map(elem =>
                  <div key={elem.title+Math.random()} className="border-bottom margin-20 padding-10">
                    <h3>{elem.title}</h3>
                    <div>
                      <span className="text-warning margin-right-5">
